...
class EAjaxUpload extends CWidget
{
    ...
    public function run()
    {
        ...
        if(!empty($this->filesName)){
            $arrImages=explode(',',$this->filesName);
            $listImages='';
            foreach($arrImages as $images){
                $listImages.="<li class=' qq-upload-success'>
                    <span class='qq-upload-file'>".$images."</span>
                    <img class='delete' src='".baseUrl()."/images/delete.png' />
                </li>";
            }  
            $html='<div class="qq-uploader">
            <div class="qq-upload-drop-area"><span>Drop files here to upload</span></div>
            <div class="qq-upload-button btn">'.Language::t(Yii::app()->language,'Frontend.Common.Common','Select').'</div><ul class="qq-upload-list">'.$listImages.'</ul></div>';
