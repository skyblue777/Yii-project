<?php

/**
 * This is the model class for table "article".
 *
 * The followings are the available columns in table 'article':
 * @property integer $id
 * @property integer $category_id
 * @property string $title
 * @property string $alias
 * @property string $leading_text
 * @property string $content
 * @property string $photo
 * @property string $tags
 * @property integer $status
 * @property integer $create_time
 * @property integer $update_time
 * @property integer $author_id
 */
class ArticleBase extends CActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @return Article the static model class
	 */
	public static function model($className='Article')
	{
		return parent::model($className);
	}

	/**
	 * @return string the associated database table name
	 */
	public function tableName()
	{
		return 'article';
	}

	/**
	 * @return array validation rules for model attributes.
	 */
	public function rules()
	{
		// NOTE: you should only define rules for those attributes that
		// will receive user inputs.
		return array(
			array('title, alias, leading_text, status, author_id, lang', 'required'),
			array('category_id, status, create_time, update_time, author_id', 'numerical', 'integerOnly'=>true),
			array('title, alias', 'length', 'max'=>512),
			array('photo', 'length', 'max'=>256),
			array('content, tags', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, category_id, title, alias, leading_text, content, photo, tags, status, create_time, update_time, author_id', 'safe', 'on'=>'search'),
		);
	}

	/**
	 * @return array relational rules.
	 */
	public function relations()
	{
		// NOTE: you may need to adjust the relation name and the related
		// class name for the relations automatically generated below.
		return array(
		);
	}

	/**
	 * @return array customized attribute labels (name=>label)
	 */
	public function attributeLabels()
	{
		return array(
			'id' => Language::t(Yii::app()->language,'Backend.Common.Common','ID'),
			'lang' => Language::t(Yii::app()->language,'Backend.Common.Menu','Language'),
			'category_id' => Language::t(Yii::app()->language,'Backend.Common.Common','Category'),
			'title' => Language::t(Yii::app()->language,'Backend.Common.Common','Title'),
			'alias' => Language::t(Yii::app()->language,'Backend.Common.Common','Alias'),
			'leading_text' => Language::t(Yii::app()->language,'Backend.Article.Common','Leading Text'),
			'content' => Language::t(Yii::app()->language,'Backend.Common.Common','Content'),
			'photo' =>  Language::t(Yii::app()->language,'Backend.Common.Common','Photo'),
			'tags' => Language::t(Yii::app()->language,'Backend.Common.Common','Tags'),
			'status' => Language::t(Yii::app()->language,'Backend.Common.Common','Status'),
			'create_time' => Language::t(Yii::app()->language,'Backend.Common.Common','Create Time'),
			'update_time' => Language::t(Yii::app()->language,'Backend.Common.Common','Update Time'),
			'author_id' => Language::t(Yii::app()->language,'Backend.Common.Common','Author'),
		);
	}

	/**
	 * Retrieves a list of models based on the current search/filter conditions.
	 * @return CActiveDataProvider the data provider that can return the models based on the search/filter conditions.
	 */
	public function search()
	{
		// Warning: Please modify the following code to remove attributes that
		// should not be searched.

		$criteria=new CDbCriteria;

		$criteria->compare('id',$this->id);
		$criteria->compare('category_id',$this->category_id);
		$criteria->compare('title',$this->title,true);
		$criteria->compare('alias',$this->alias,true);
		$criteria->compare('leading_text',$this->leading_text,true);
		$criteria->compare('content',$this->content,true);
		$criteria->compare('photo',$this->photo,true);
		$criteria->compare('tags',$this->tags,true);
		$criteria->compare('status',$this->status);
		$criteria->compare('create_time',$this->create_time);
		$criteria->compare('update_time',$this->update_time);
		$criteria->compare('author_id',$this->author_id);

		return new CActiveDataProvider(get_class($this), array(
			'criteria'=>$criteria,
		));
	}
}