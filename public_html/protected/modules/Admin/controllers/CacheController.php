...
class CacheController extends BackOfficeController
{
	/**
	...
	public function actionView($id)
	{
		$this->render('view',array(
			'model'=>FSM::run('Admin.Cache.get', array('id'=>$id))->model,
		));
	}
	...
	public function actionUpdate()
	{
        if (Yii::app()->request->IsPostRequest) {
            // save posted data
            $_POST['validateOnly'] = ($this->post('ajax','') == 'cache-form');
            $result = FSM::run('Admin.Cache.save', $_POST);
            $model = $result->model; 

            if ($this->post('ajax','') == 'cache-form'){
	...
	public function actionDelete($id)
	{
        if(Yii::app()->request->isPostRequest)
        {
            ...
            if (($id = $this->get('id',null)) !== null) {
                ...
                $result = FSM::run('Admin.Cache.delete', array('ids' => $ids));
                if ($result->hasErrors()) {
