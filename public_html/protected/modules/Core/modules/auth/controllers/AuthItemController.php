...
* @since 0.5
*/
class AuthItemController extends BackOfficeController
{
	/**
	* @var RightsModule
	*/
...
//	}
	...
	*/
	public function actionCreate()
	{
		...
		if( isset($_GET['type'])===true )
		{
		    ...
		    $form = new CForm('auth.views.authItem.authItemForm', new AuthItemForm('create'));
		    ...
		    if( $form->submitted()===true && $form->validate()===true )
			{
				...
				Yii::app()->user->setFlash($this->_module->flashSuccessKey,
					Yii::t('AuthModule.core', ':name created.', array(':name'=>Rights::beautifyName($form->model->name)))
				);
		...
		{
			throw new CHttpException(404, Language::t(Yii::app()->language,'Backend.Common.Message','Invalid authorization item type.'));
		}
	...
	*/
	public function actionUpdate()
	{
		...
		// Create the authorization item form
	    $form = new CForm('auth.views.authItem.authItemForm', new AuthItemForm('update'));
		...
		if( $form->submitted()===true && $form->validate()===true )
		{
			...
			Yii::app()->user->setFlash($this->_module->flashSuccessKey,
				Yii::t('AuthModule.core', ':name updated.', array(':name'=>Rights::beautifyName($form->model->name)))
			);
		...
		if( $selectOptions!==array() )
		{
		    ...
		    $childForm = new CForm('auth.views.authItem.authChildForm', new AuthChildForm);
			...
			if( $childForm->submitted()===true && $childForm->validate()===true )
			{
				...
				Yii::app()->user->setFlash($this->_module->flashSuccessKey,
					Yii::t('AuthModule.core', 'Child :name added.', array(':name'=>Rights::beautifyName($childForm->model->name)))
				);
	...
	public function actionDelete()
	{
		...
		if( Yii::app()->request->isPostRequest===true )
		{
			...
			Yii::app()->user->setFlash($this->_module->flashSuccessKey,
				Yii::t('AuthModule.core', ':name deleted.', array(':name'=>Rights::beautifyName($_GET['name'])))
			);
		...
		{
			throw new CHttpException(400, Language::t(Yii::app()->language,'Backend.Common.Message','Invalid request. Please do not repeat this request again.'));
		}
	}
	...
	*/
	public function actionGenerate()
	{
		...
		$model = Yii::createComponent('auth.models.GenerateForm');
...
//						Yii::t('AuthModule.core', 'Authorization items created.')
//					);
                    $this->message = Yii::t('AuthModule.core', 'Authorization items created.');
//					$this->redirect(array('default/permissions'));
	...
	*/
	public function actionRemoveChild()
	{
		...
		if( Yii::app()->request->isPostRequest===true )
		{
			...
			Yii::app()->user->setFlash($this->_module->flashSuccessKey,
				Yii::t('AuthModule.core', 'Child :name removed.', array(':name'=>Rights::beautifyName($_GET['child'])))
			);
	...
	*/
	public function actionRevoke()
	{
		...
		{
			throw new CHttpException(400, Yii::t('AuthModule.core', 'Invalid request. Please do not repeat this request again.'));
		}
	}
