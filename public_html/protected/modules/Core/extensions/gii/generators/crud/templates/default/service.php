...
class <?php echo $this->modelClass; ?>APIService extends FServiceBase
{    
    /**
    ...
    */
    public function get($params){
        ...
        if (! $model)
            $this->result->fail(ERROR_INVALID_DATA, Yii::t('<?php echo $moduleId; ?>.<?php echo $this->modelClass; ?>','Invalid ID.'));
    ...
    public function save($params) {
        ...
        if (! $model->validate()) {
            $this->result->fail(ERROR_INVALID_DATA, Yii::t('<?php echo $moduleId; ?>.<?php echo $this->modelClass; ?>', 'Submitted data is missing or invalid.'));
            ...
            $this->result->fail(ERROR_HANDLING_DB, Yii::t('<?php echo $moduleId; ?>.<?php echo $this->modelClass; ?>','Error while saving submitted data into database.'));
        }
        return $this->result;
    }
    ...
    public function delete($params) {
        ...
        if ($ids == 0) {
            return $this->result->fail(ERROR_INVALID_DATA, Yii::t('<?php echo $moduleId; ?>.<?php echo $this->modelClass; ?>','Invalid ID.'));
        }
        ...
        foreach($ids as $id) {
            ...
            *
            if (Related::model()->count("<?php echo $this->modelClass; ?>Id = {$id}") > 0)
                $this->result->fail(ERROR_VIOLATING_BUSINESS_RULES, Yii::t('<?php echo $moduleId; ?>.<?php echo $this->modelClass; ?>',"Cannot delete <?php echo $this->modelClass; ?> ID={$id} as it has related class data."));
            else
            */
                try {
                    $model->delete();
